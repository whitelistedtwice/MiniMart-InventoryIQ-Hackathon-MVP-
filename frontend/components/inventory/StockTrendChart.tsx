/*
  Simple historical stock-level chart.

  Maps the backend's `historical_inventory` pairs into a lightweight SVG
  line. No forecasting, no new calculations — only axis scaling for display.
  Missing/short history is handled with a clear message instead of a fake chart.
*/
...
export function StockTrendChart({ points }: { points: Point[] }) {
  if (points.length < 2) {
    return (
      <p className="py-10 text-center text-sm text-muted">
        Not enough stock history to show a trend yet.
      </p>
    );
  }
  ...
  return (
    <figure>
      <svg
        ...
        role="img"
        aria-label={`Stock level from ${firstDate} to ${lastDate}, ranging between ${min} and ${max} units`}
      ...
      <figcaption className="mt-2 text-xs text-muted">
        Observed stock on hand over time (units).
      </figcaption>
    </figure>
  );
}
